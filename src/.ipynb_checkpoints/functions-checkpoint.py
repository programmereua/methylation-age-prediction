...
def check_missing_values(df):
    ...
    missing_sorted = missing.sum().sort_values(ascending=False)
...
def plot_bootstrap_boxplots(results_dict):
    ...
    axes[1].set_ylabel("R² (coefficient of determination)")
...
def plot_frequency(counts):
    ...
    plt.xlabel("Times selected (out of 50 resamples)")
...
def choose_best_k(train_df, val_df):
    ...
    # impute missing values before mRMR to avoid NaN issues
    imputer = SimpleImputer(strategy="median")
    X_train_cpg = pd.DataFrame(
        imputer.fit_transform(train_df[cpg_cols]),
        columns=cpg_cols,
    ...
    for K in k_values:
        ...
        # run mRMR on the imputed training data
        selected = mrmr_regression(X=X_train_cpg, y=train_df["age"], K=K)
...
def run_mrmr(train_df, K):
    ...
    # run mRMR - finds K features that predict age but are different from each other
    selected_features = mrmr_regression(X=X_train_cpg, y=y_train, K=K)
    
    # top 10
    print(f"mRMR selected {K} features")
    print("Top 10 features:")
    for i, feat in enumerate(selected_features[:10]):
        corr, _ = spearmanr(X_train_cpg[feat], train_df["age"])  
...
def plot_predicted_vs_actual(y_true, y_pred, model_name):
    ...
    plt.scatter(y_true, y_pred, alpha=0.6, color='steelblue', edgecolor='black')
    ...
    plt.title(f'Predicted vs Actual Age - {model_name}')
    plt.legend()
    plt.savefig(f'predicted_vs_actual_{model_name}.png', dpi=150, bbox_inches='tight')
    plt.show()
#BONUS A
...
def select_sex_features(train_df, K):
    ...
    # impute missing values before mRMR - same as we do for age
    imputer = SimpleImputer(strategy="median")
    X_train_cpg = pd.DataFrame(
        imputer.fit_transform(train_df[cpg_cols]),
        columns=cpg_cols,
    ...
    sex_features = mrmr_classif(X=X_train_cpg, y=train_df["sex_label"], K=K)
...
def plot_confusion_matrices(lr, gnb, X_eval, y_eval):
    
    # we will make 2 plots side by side - one for each model
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    ...
    for ax, model, name in zip(axes, [lr, gnb], ['Logistic Regression', 'Gaussian NB']):
        ...
        # now lets count how many we got right and wrong
        # tp = we said Male and it was Male - correct!
        # tn = we said Female and it was Female - correct!
        # fp = we said Male but it was Female - oops
        # fn = we said Female but it was Male - oops
        tp = int(((y_pred == 1) & (y_eval == 1)).sum())
        tn = int(((y_pred == 0) & (y_eval == 0)).sum())
        fp = int(((y_pred == 1) & (y_eval == 0)).sum())
        fn = int(((y_pred == 0) & (y_eval == 1)).sum())
        
        # put the counts in a little 2x2 table
        matrix = [[tn, fp],
                  [fn, tp]]
        
        # draw it as a colored grid - darker = bigger number
        ax.imshow(matrix, cmap='Blues')
        ax.set_xticks([0, 1])
        ax.set_yticks([0, 1])
        ax.set_xticklabels(['Predicted F', 'Predicted M'])
        ax.set_yticklabels(['Actual F',    'Actual M'])
        ax.set_title(f'Confusion Matrix - {name}')
        
        # write the actual numbers inside each box so we can read them
        for i in range(2):
            for j in range(2):
...
def plot_roc_curves(lr, gnb, X_eval, y_eval):
    
    plt.figure(figsize=(7, 6))
    
    # we will plot one curve for each model
    models = [lr,                    gnb             ]
    ...
    for model, name, color in zip(models, names, colors):
        ...
        # get the probability that each sample is Male
        # closer to 1.0 = more confident it is Male
        y_prob = model.predict_proba(X_eval)[:, 1]
        
        # AUC score - 1.0 is perfect, 0.5 is random guessing
        auc = roc_auc_score(y_eval, y_prob)
        
        # calculate the curve by trying different thresholds
        thresholds = sorted(set(y_prob), reverse=True)
        tpr_list   = []  # true positive rate  = % of Males we correctly caught
        fpr_list   = []  # false positive rate = % of Females we wrongly called Male
        
        for thresh in thresholds:
            y_pred_t = (y_prob >= thresh).astype(int)
            tp = int(((y_pred_t == 1) & (y_eval == 1)).sum())
            fp = int(((y_pred_t == 1) & (y_eval == 0)).sum())
            fn = int(((y_pred_t == 0) & (y_eval == 1)).sum())
            tn = int(((y_pred_t == 0) & (y_eval == 0)).sum())
            tpr_list.append(tp / (tp + fn) if (tp + fn) > 0 else 0)
            fpr_list.append(fp / (fp + tn) if (fp + tn) > 0 else 0)
        
        plt.plot(fpr_list, tpr_list, color=color,
                 label=f'{name} (AUC = {auc:.3f})')
    
    # this line shows what a completely random model would look like
...
def plot_top20_sex_cpgs(train_df):
    
    # get all CpG columns
    cpg_cols = [col for col in train_df.columns if col.startswith("cg")]
    y        = train_df["sex_label"].values
    
    # for each CpG site we calculate how strongly it is linked to sex
    # we use pearson correlation - since sex_label is 0 or 1 this is called point-biserial r
    print("Calculating correlation of each CpG with sex...")
    correlations = {}
    for col in cpg_cols:
        # fill any missing values with the median before calculating
        x             = train_df[col].fillna(train_df[col].median()).values
        r, _          = pearsonr(x, y)
        correlations[col] = abs(r)  # we only care about the size, not the direction
    
    # sort all CpGs and keep only the best 20
    top20      = sorted(correlations, key=correlations.get, reverse=True)[:20]
    top20_vals = [correlations[col] for col in top20]
    
    print("Top CpG:", top20[0], "with r =", round(top20_vals[0], 4))
    
    # plot a horizontal bar chart - longer bar = stronger link to sex
    plt.figure(figsize=(10, 6))
    plt.barh(top20[::-1], top20_vals[::-1],
             color='steelblue', edgecolor='black')
    plt.xlabel('|Point-biserial r| with sex')
    plt.title('Top 20 CpGs that best separate Males from Females')
    plt.tight_layout()
    plt.savefig('top20_sex_cpgs.png', dpi=150, bbox_inches='tight')
    plt.show()

def plot_classifier_boxplots(lr_scores, gnb_scores):
    
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    
    metrics = [
        ('acc_scores', 'Accuracy',  'Accuracy (0-1)'),
        ('f1_scores',  'F1 Score',  'F1 Score (0-1)'),
        ('mcc_scores', 'MCC',       'MCC (-1 to 1)'),
        ('roc_scores', 'ROC-AUC',   'ROC-AUC (0-1)')
    ]
    
    for ax, (key, title, ylabel) in zip(axes.flatten(), metrics):
        
        # put the bootstrap scores for both models side by side
    ...
    plt.tight_layout()
    plt.savefig('classifier_boxplots.png', dpi=150, bbox_inches='tight')
    plt.show()
